...
### Performance
The accuracy is calculated by **Quadratic Weighted Kappa(QWK)**, which measures the agreement between two raters. The state-of-the-art implementation in this competition achieved a **QWK score of 0.82** six years ago. My model achieves a **QWK score of 0.80** combined of 8 topics. The model architecture consists of 2 MLP (multi-layer perceptron) layers with a Dense output layer. The final layer uses the Relu activation function. The QWK is calculated by training the model on the dataset using 5-fold cross-validation and taking the average for all five folds.
